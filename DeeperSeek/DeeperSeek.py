...
class DeepSeek:
    ...
    async def _login(self) -> None:
        ...
        # Delay to ensure the token login has time to process
        await sleep(2)
    ...
    async def _find_child_by_text(
        ...
        """Finds a child element by its text.
    ...
    async def _get_response(
        ...
        self.logger.debug("Waiting for the response to start generating..." if not regen else "Waiting for the response to start regenerating...")
        while time() < end_time:
            try:
                _ = await self.browser.main_tab.select(self.selectors.backend.response_generating if not regen else self.selectors.backend.regen_loading_icon)
            except:
                continue
            else:
                break
        ...
        self.logger.debug("Waiting for the response to finish generating..." if not regen else "Finding the last response...")
        while time() < end_time:
            try:
                response_generated: zendriver.Element = await self.browser.main_tab.select_all(self.selectors.backend.response_generated)
            except:
                continue
